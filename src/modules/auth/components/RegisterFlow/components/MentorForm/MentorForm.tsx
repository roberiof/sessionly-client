...
export function MentorForm({ onSuccess, onBack }: Props) {
  ...
  async function handleNext() {
    const valid = await trigger(["name", "bio", "niche", "specialties"])
    if (valid) {
      setDirection(1)
      setSubStep(2)
    }
  ...
  function onSubmit(data: MentorRegisterFormData) {
    const { niche, specialties: sp, confirmPassword: _, ...rest } = data
    createUser({ ...rest, mentorProfile: { niche, specialties: sp } })
  }
        ...
        <AnimatePresence mode="wait" initial={false}>
          {subStep === 1 ? (
            <motion.div
              ...
              className="flex flex-col gap-4"
            >
              {[{ name: "name" as const, label: "Full name", delay: 0.04 }].map(
                ({ name, label, delay }) => (
                  <motion.div
                    key={name}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <Textarea
                      {...field}
                      id="bio"
                      placeholder="e.g. Senior frontend engineer with 8 years experience in React…"
                      autoComplete="off"
                      rows={3}
                    />
                  )}
              ...
              <motion.div
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3, delay: 0.16, ease: EASE_EXPO }}
              >
                <FormField
                  name="specialties"
                  control={control}
